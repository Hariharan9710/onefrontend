...
const ProjectAssignment: React.FC = () => {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await adminApi.get(`/project-assignments/project/${projectId}`);
      const result = response.data;
      if (result.success) {
  ...
  const fetchEmployeesByDepartment = async (departmentId: number) => {
    try {
      ...
      const endpoints = [
        `/project-assignments/department/${departmentId}/employees`,
      ...
      if (employeesData.length === 0) {
        try {
          ...
          console.error("General employees fallback failed:", err);
        }
      }
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      setSubmitting(true);
      setError('');
      const response = await adminApi.post("/project-assignments/assign-multiple", {
        project_id: projectId,
  ...
  const handleDeleteAssignment = async (assignId: number) => {
    try {
      const response = await adminApi.delete(`/project-assignments/${assignId}`);
      const result = response.data;
