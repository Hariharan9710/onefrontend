...
const PayrollForm: React.FC = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await adminApi.get('/payroll/employees');
  ...
  const fetchPayroll = async () => {
    try {
      const response = await adminApi.get(`/payroll/${id}`);
  ...
  const handleCalculate = async () => {
    ...
    setCalculating(true);
    try {
      const response = await adminApi.post('/payroll/calculate',
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminApi[isEditMode ? 'put' : 'post'](path, formData);
